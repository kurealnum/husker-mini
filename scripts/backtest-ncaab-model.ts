/**
 * Fits and backtests a candidate NCAAB-specific win-probability model
 * against real, completed games, to decide whether NCAAB needs its own
 * coefficients or can share NBA's. Same methodology as
 * `scripts/backtest-basketball-model.ts`. NCAAB has 360+ D1 teams — far
 * more than fits in a quick run at ESPN's request pace — so this samples
 * the first `TEAM_SAMPLE_SIZE` teams (alphabetical by ESPN's team list
 * order, not seeded/ranked) rather than fetching every team.
 *
 * Run with: npx tsx scripts/backtest-ncaab-model.ts
 */

export {};

const SITE_API_BASE = "https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball";

/** The most recent fully-completed NCAAB season, used since the current season has too few played games to fit against. */
const BACKTEST_SEASON = 2025;

/** Bounds runtime against ESPN's per-request pacing — see file doc comment. */
const TEAM_SAMPLE_SIZE = 120;
...
async function main() {
  console.error("Fetching NCAAB team ids (sampled)...");
  const teamIds = (await fetchTeamIds()).slice(0, TEAM_SAMPLE_SIZE);
  console.error(`${teamIds.length} teams (sampled from the full D1 list).`);
