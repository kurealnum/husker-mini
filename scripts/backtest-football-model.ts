/**
 * Fits and backtests the football win-probability model against real,
 * completed NFL games this season. For each team, walks its schedule
 * chronologically and computes each game's pre-game "eloDiff" feature from
 * only the scoring differential of games strictly before it (no
 * future-game leakage), then fits a 1-D logistic regression via gradient
 * descent and reports in-sample accuracy.
 *
 * This intentionally reuses only the scoring-differential half of the live
 * pipeline's opponent-adjusted-strength calculation (skips the
 * opponent-of-opponent adjustment and player-rating feature) — enough
 * data to fit and sanity-check the single feature that's cheap to compute
 * for all 32 teams without also fetching every team's roster/gamelogs.
 * `playerRatingDiff` ships with a zero weight until that's worth the
 * additional fetch cost, exactly like MLB's still-zero savePctDiff/opsDiff/
 * eraDiff (src/lib/win-probability-model.ts).
 *
 * Run with: npx tsx scripts/backtest-football-model.ts
 */

export {};

const SITE_API_BASE = "https://site.api.espn.com/apis/site/v2/sports/football/nfl";
...
/** The most recent fully-completed NFL season, used since the current season has too few played games to fit against. */
...
async function main() {
  console.error("Fetching NFL team ids...");
